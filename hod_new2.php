...
global $connection;
 $service="";
 $ser_query="select name from service where id=$ser"; 
 $ser_query_run=mysqli_query($connection,$ser_query);
 while ($result=mysqli_fetch_array($ser_query_run)) {
  $service=$result['name'];
 }
 return $service;
}
...
$query= "select count(th.ticket_id) as count,substring_index(substring_index((select name as name
                          from ticket_history
                            where id = (
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <= ".$date." and date(x.change_time) >= '2018-02-01' 
                            ) ), '%', 5),'%',-1) as ser_id,s2.name as service,th.state_id
              from ticket_history th
               inner join service s2 on s2.id=
              substring_index(substring_index((select name as name
                          from ticket_history
                            where id = (
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <= ".$date." and date(x.change_time) >= '2018-02-01' 
                  ...
                  on h.ticket_id=ti.id  
                    ...
                    inner join users u1 on t.responsible_user_id=u1.id
                      ...
                      and t.customer_id not in('phl-ubp') and t.create_time>='2018-02-01'
                      group by ser_id
                      order by count desc";



$query2="select count(th.ticket_id) as count,substring_index(substring_index((select name as name
                          from ticket_history
                            where id = (
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <= DATE_SUB(".$date.",INTERVAL 1 WEEK) and date(x.change_time) >= '2018-02-01' 
                            ) ), '%', 5),'%',-1) as ser_id,s2.name as service,th.state_id
              from ticket_history th
               inner join service s2 on s2.id=
              substring_index(substring_index((select name as name
...
array_push($serv,$result_set['ser_id']);

}

while ($result2_set=mysqli_fetch_array($query2_run)) { 

array_push($serv,$result2_set['ser_id']);
  ...
  $table_list.='<table class="hodlist" id="myTable" border="1" style="border-collapse: collapse;">  
      <thead>
      <tr class="head">  
        <th rowspan="2" id="hod">Product</th>      
...
foreach ((array_unique($serv)) as $ser_id) {
//echo $ser_id.'<br>';


    $query_current_ibl="select count(th.ticket_id) as count,substring_index(substring_index((select name as name
                          from ticket_history
                            where id = (
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <=  ".$date." and date(x.change_time) >= '2018-02-01' 
                          ...
                          from ticket_history
                            ...
                            and date(x.change_time) <=  ".$date." and date(x.change_time) >= '2018-02-01' 
                                  ...
                                  on h.ticket_id=ti.id  
                                    ...
                                    inner join ticket t on t.id = th.ticket_id
                                    inner join service s1 on t.service_id=s1.id
                                      where substring_index(substring_index((select name as name
                          from ticket_history
                            where id = (
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <=  ".$date." and date(x.change_time) >= '2018-02-01' 
                            ) ), '%', 5),'%',-1)=$ser_id and th.state_id  in ('1','4','12','20','23','26','27','28','29','32') and                 
                                        t.customer_id not in('phl-ubp') and t.queue_id not in(41)
										and t.create_time>='2018-02-01'";
    ...
    $query_current_ibl_run=mysqli_query($connection,$query_current_ibl);
     while($result_set_current_ibl=mysqli_fetch_array($query_current_ibl_run)){
      $table_list.='<tr class="data"><td class="res">'.a($ser_id).'</td><td class="center">'. $ibl_cur=$result_set_current_ibl['count'].'</td>';
     }    
  
    $query_previous_ibl="select count(th.ticket_id) as count,substring_index(substring_index((select name as name
                          from ticket_history
                            where id = (
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <= DATE_SUB(".$date.",INTERVAL 1 WEEK) and date(x.change_time) >= '2018-02-01' 
    ...
    $query_previous_ibl_run=mysqli_query($connection,$query_previous_ibl);
     ...
     $query_current_info="select count(th.ticket_id) as count,substring_index(substring_index((select name as name
                          from ticket_history
                            where id = (
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <=  ".$date." and date(x.change_time) >= '2018-02-01' 
                          ...
                          from ticket_history
                            ...
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <=  ".$date." and date(x.change_time) >= '2018-02-01' 
                            ) ), '%', 5),'%',-1)=$ser_id and th.state_id  in (14,30,31) and                 
                                        t.customer_id not in('phl-ubp') and t.queue_id not in(41)
										and t.create_time>='2018-02-01'";
     ...
     $query_previous_info="select count(th.ticket_id) as count,substring_index(substring_index((select name as name
                          from ticket_history
                            where id = (
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <= DATE_SUB(".$date.",INTERVAL 1 WEEK) and date(x.change_time) >= '2018-02-01' 
     ...
     $query_current_hold="select count(th.ticket_id) as count,substring_index(substring_index((select name as name
                          from ticket_history
                            where id = (
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <=  ".$date." and date(x.change_time) >= '2018-02-01' 
                          ...
                          from ticket_history
                            ...
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <=  ".$date." and date(x.change_time) >= '2018-02-01' 
                            ) ), '%', 5),'%',-1)=$ser_id and th.state_id  in (13) and                 
                                        t.customer_id not in('phl-ubp') and t.queue_id not in(41)
										and t.create_time>='2018-02-01'";
   ...
   $query_previous_hold="select count(th.ticket_id) as count,substring_index(substring_index((select name as name
                          from ticket_history
                            where id = (
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <= DATE_SUB(".$date.",INTERVAL 1 WEEK) and date(x.change_time) >= '2018-02-01' 
     ...
     $query_current_release="select count(th.ticket_id) as count,substring_index(substring_index((select name as name
                          from ticket_history
                            where id = (
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <=  ".$date." and date(x.change_time) >= '2018-02-01' 
                          ...
                          from ticket_history
                            ...
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <=  ".$date." and date(x.change_time) >= '2018-02-01' 
                            ) ), '%', 5),'%',-1)=$ser_id and th.state_id  in (33) and                 
                                        t.customer_id not in('phl-ubp') and t.queue_id not in(41)
										and t.create_time>='2018-02-01'";
    $query_current_release_run=mysqli_query($connection,$query_current_release);
     while($result_set_release_run=mysqli_fetch_array($query_current_release_run)){
      ...
      $query_previous_release="select count(th.ticket_id) as count,substring_index(substring_index((select name as name
                          from ticket_history
                            where id = (
                            select max(id) from ticket_history x where history_type_id = 38 and ticket_id = th.ticket_id
                            and date(x.change_time) <= DATE_SUB(".$date.",INTERVAL 1 WEEK) and date(x.change_time) >= '2018-02-01' 
     ...
     while($result_set_previous_release=mysqli_fetch_array($query_previous_release_run)){
      ...
      $table_list.='<td class="center">'.$efe_cur.'</td><td class="previous">'.$efe_prv.'</td><td class="change">'.$efe_chg.'</td>';
...
$query_pool="select t.tn,t.title,ts.name as state,tp.name as priority,t.customer_id,s2.name as service,u.first_name as owner,
u1.first_name as responsible
              from ticket_history th
              inner join service s2 on s2.id=
             substring_index(substring_index((select name as name
                          from ticket_history
...
<header>
  ...
  <form name="hod" method="POST" action="hod_new2.php">
    
    <div style=" margin: 0 200 auto;"ng-app="myApp" ng-controller="myCntrl">
    <h4 style=" margin: 0 150 auto;">Onging Product wise</h4>
    Select Date:
...
var result = [];
  $('#myTable tr').each(function(){
    $('td:not(.res)', this).each(function(index, val){
      if(!result[index]) result[index] = 0;
      result[index] += parseInt($(val).text());
    });
  });
  
  $('#myTable').append('<tr></tr><td>Total</td>');
  $(result).each(function(){
    $('#myTable tr').last().append('<td>'+this+'</td>')
  });
...
// Load the Visualization API and the corechart package.
   //google.charts.load('current', {'packages':['corechart']});
    google.charts.load('current', {packages: ['corechart', 'bar']});
      ...
      google.charts.setOnLoadCallback(iblbar);
      google.charts.setOnLoadCallback(effectivebar);
...
//alert(test2);
 ...
 data.addRows([[hod, count]]);
        ...
        // Set chart options
        var options = {'title':'Ongoing IBL Pending',
                       'width':700,
                       'height':700};
...
//alert(test2);
 ...
 data.addRows([[hod, count]]);
        ...
        // Set chart options
        var options = {'title':'Ongoing Effective CSR',
                       'width':700,
                       'height':700};
...
//alert(test2);
 ...
 data.addRows([[hod, count]]);
        ...
        // Instantiate and draw our chart, passing in some options.
        var chart = new google.visualization.ColumnChart(document.getElementById('ibl_bar'));
        chart.draw(data, options);
      }


function effectivebar() {
...
//alert(test2);
 ...
 data.addRows([[hod, count]]);
        ...
        var options = {'title':'Effective CSR',
                       'width':1200,
                       'height':1200,
                      colors: ['#3366CC','#DC3912','#FF9900','#109618','#990099','#3B3EAC','#DD4477','#66AA00','#B82E2E','#316395','#994499','#22AA99','#6633CC','#E67300','#8B0707','#329262','#5574A6','#3B3EAC'], };

        // Instantiate and draw our chart, passing in some options.
        var chart = new google.visualization.ColumnChart(document.getElementById('eff_bar'));
        chart.draw(data, options);
      }
...
<div>
<div style="float:left; width:50%;" class="left" id="chart_div"></div>
<div style="float:left; width:50%;" class="left" id="chart_div2"></div>
</div>

<div style="float:left; width:100%;" class="left" id="ibl_bar"></div> 
<div style="float:left; width:100%;" class="left" id="eff_bar"></div> 
