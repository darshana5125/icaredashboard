...
<body>    
    <div class="container">        
        ...
        <?php $tile="";
        $tile.='<div class="row" style="margin-top:20px;">
                <div class="col-md-6">                                
    ...
    $table.='<table style="font-family:arial;font-size:11px;margin: 0 auto;" id="example" class="table table-sm table-bordered" x>
...
if(isset($_POST['customer']) && isset($_POST['service'])){
    ...
    $ticket_no_list_query="select xxx.ticket_id,t.customer_id,t.tn,t.title,t.create_time,t.service_id,
                        ...
                        t.create_time between '".$create_frm_date."' and '".$create_to_date."'";
    ...
    $ticket_no_list_query="select xxx.ticket_id,t.customer_id,t.tn,t.title,t.create_time,t.service_id,
                        ...
                        inner join service_view s on
                        t.service_id=s.id
                        where t.customer_id in (".$cx_list.") and t.create_time between '".$create_frm_date."' and '".$create_to_date."'";
    ...
    $ticket_no_list_query="select xxx.ticket_id,t.customer_id,t.tn,t.title,t.create_time,t.service_id,
                        ...
                        inner join service_view s on
                        t.service_id=s.id
                        where t.service_id in (".$service_list.") and t.create_time between '".$create_frm_date."' and '".$create_to_date."'";    
}else{        
$ticket_no_list_query="select xxx.ticket_id,t.customer_id,t.tn,t.title,t.create_time,t.service_id,
                        tp.name as priority,s.name as service from temp_max_id3 xxx
                        inner join issue_view t on
                        ...
                        inner join service_view s on
                        t.service_id=s.id
                        where t.create_time between '".$create_frm_date."' and '".$create_to_date."'";
}
...
while($result3=mysqli_fetch_array($query3_run)){
    ...
    if($history_type_id==27 && $change_by==1 && $state_id==4){
        ...
        else if($tech_assign_flag==0){
            $tech_assign_flag=1;
            //$tech_unassign_time=$result3['change_time'];
            //echo 'Tech unassign time 3333 '.$tech_unassign_time.'<br>';
            //$tech_time_in_seconds+=strtotime($tech_unassign_time) - strtotime($tech_assign_time);
            $tech_assign_time=$result3['change_time'];
...
// end of finding the ticket age of a given ticket
    ...
    $table.="<tr>
            <td>". $count."</td>
            <td>".$ticket_no_list_query_result['customer_id']."</td>
            <td><a href='https://icare.interblocks.com/otrs/index.pl?Action=AgentTicketZoom;TicketID=".$ticket_no_list_query_result['ticket_id']."' target='_blank'>".$ticket_no_list_query_result['tn']."</a></td>
            <td>".substr($ticket_no_list_query_result['title'],0,100)."</td>
            <td>".date('Y:m:d', strtotime($ticket_no_list_query_result['create_time']))."</td>
            <td>".$ticket_no_list_query_result['service']."</td>
            <td>".$ticket_no_list_query_result['priority']."</td>
            <td><input type='hidden' value='".$hd_time_in_seconds."'>".secondsToTime($hd_time_in_seconds)."</td>
            <td><input type='hidden' value='".$tech_time_in_seconds."'>".secondsToTime($tech_time_in_seconds)."</td>
            <td><input type='hidden' value='".$cx_time_in_seconds."'>".secondsToTime($cx_time_in_seconds)."</td>
        </tr>";        
    ?>
<?php
$count++;
