    ...
    <div class="container">
        <div class="row">
            <div class="col-md-10">
                ...
                $query = "select id,a_subject as subject from article";
                ...
                while ($result = mysqli_fetch_array($query_run)) {
                    $id = $result['id'];
                    //$tn = $result['tn'];
                    $subject =  mysqli_real_escape_string($connection,$result['subject']);
                    $regex = '/(?:\d[ \t-]*?){13,19}/m';
                    $matches = [];
                    preg_match_all($regex, $subject, $matches);
                    if (!empty($matches[0])) {
                        $masksubject = $magoo->getMasked($subject . PHP_EOL);
                        //echo $count."     ". $masktitle.'<br>';
                        //$count++;
                        $update_query = "update article set a_subject='" . $masksubject . "' where id=" . $id;
