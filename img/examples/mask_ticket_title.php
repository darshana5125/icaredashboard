    ...
    <div class="container">
        <div class="row">
            <div class="col-md-10">
                ...
                $query = "select id,tn,title from ticket";
                ...
                while ($result = mysqli_fetch_array($query_run)) {
                    $id = $result['id'];
                    $tn = $result['tn'];
                    $title =  mysqli_real_escape_string($connection,$result['title']);
                    $regex = '/(?:\d[ \t-]*?){13,19}/m';
                    $matches = [];
                    preg_match_all($regex, $title, $matches);
                    if (!empty($matches[0])) {
                        $masktitle = $magoo->getMasked($title . PHP_EOL);
                        //echo $count."     ". $masktitle.'<br>';
                        //$count++;
                        $update_query = "update ticket set title='" . $masktitle . "' where id=" . $id;
