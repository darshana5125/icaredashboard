...
echo '<link rel="stylesheet" href="\icaredashboard/libraries/googleapis/font.css" >
 <link href="\icaredashboard/css/main3.css" rel="stylesheet">
...
<form method="POST" action="sla_update.php?mid=5">
...
$csr_query="select distinct * from view_temp2 where tn='$csr_no'";
...
function date() {
 ...
 <option value="1">1 SLA Met </option>
  ...
  echo '<label class="day" id="dtlabel" style="display:none">Select a Date</label>
  <input type="text" id="datepicker"  name="datepicker" style="display:none"/> </div>';
  ...
  else
  {
	...
	window.location.href = 'sla_update.php?mid=5';
</script>";
	  }
  }
...
if($sla!="sts")
	...
	//echo $breached_time_string;
	 ...
	 if($breached_time_string!=""){
		...
		//echo $breached_time;
		$query_sla= "update servicelevel_view set sla_state='$sla',updated_by='$user_id', breached_time='$breached_time',update_time=NOW() where tn='$csr'";
	}
	
	
	else{
  //$query_sla= "update view_temp2 set sla_state='$sla' where tn='$csr'";
  $query_sla= "update servicelevel_view set sla_state='$sla',updated_by='$user_id',update_time=NOW() where tn='$csr'";
	}
