...
<?php 
	...
	if (isset($_GET['user_id'])) {
		...
		$query = "SELECT * FROM user_view WHERE id = {$user_id} LIMIT 1";
	...
	if (isset($_POST['submit'])) {
		...
		$req_fields = array('user_id', 'first_name', 'email');
		...
		$query = "SELECT * FROM user_view WHERE email = '{$email}' AND id != {$user_id} LIMIT 1";
		...
		if (empty($errors)) {
			...
			$query = "UPDATE user_view SET ";
			$query .= "first_name = '{$first_name}', ";
			$query .= "user_name = '{$user_name}', ";
			$query .= "email = '{$email}', ";
			$query .= "full_access= '{$full_access}'";
...
<body>
	...
	<main>
		...
		<form action="modify-user.php" method="post" class="userform">
			...
			<p>
				<label for="">Full access:</label>
				<input type="text" name="full_access" <?php echo 'value="' . $full_access . '"'; ?>>
			</p>
