				...
				<div class="row">
					<div class="col-md-10">
						...
						<div class="input-group date">
							<form action="agent_wise2.php" method="POST">
						...
						Coalesce(SUM(issview.fix), 0)      AS fix_count 
				 		...
				 		GROUP  BY issview.customer_id";
								  ...
								  from ticket_history
									where id =  (
									select max(id) from ticket_history x where  x.history_type_id=34
									and date(x.change_time) <= '".$date."'                
									) ), '%', -1) 
									 inner join users u1 on iv.responsible_user_id=u1.id
									  and iv.responsible_user_id in (".$user_ids.")
										where iv.customer_id in('".$customer_id."') and ih.change_time<'".$date."' and 
										ih.id in (select max(y.id) from ticket_history y where y.change_time<'".$date."'
