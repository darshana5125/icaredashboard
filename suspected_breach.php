...
<?php
echo "Please note this will only list down CSR which are created on or after 2016-Jan-01.";
...
<option value="1st" >1st Escalation Pending</option>
<option value="2nd" >2nd Escalation Pending</option>
<option value="breached" selected>Suspected Breach</option>
...
<input type="hidden" id="min" value=120 />
<input type="hidden" id="max" value=50000/>
<input type="hidden" id="minh" value=24/>
<input type="hidden" id="maxh" value=50000/>
<input type="hidden" id="minc" value=2 />
<input type="hidden" id="maxc" value=50000/>
<input type="hidden" name="csrs" id="csrs">';
...
$query_rows="SELECT  distinct count(issue_view.tn) as count,issue_view.tn as tn,issuehis_view.state_id,issuehis_view.change_time,servicelevel_view.1st_esc,servicelevel_view.2nd_esc,servicelevel_view.remarks
...
where issue_view.create_time>'2016-01-01 00:00:00'
and issue_view.ticket_state_id in(1,4,14,9,12,13,20,23,28,30,31) and issue_view.queue_id not in(7,13) and issuehis_view.history_type_id=27 
and servicelevel_view.sla_state not in(1,3)
...
//{			
			...
			<th>Create Time</th><th>Priority</th><th>Remarks</th><th>Actual Age</th><th>Total Age </th><th>Adjustments</th><th>Actual Age(hrs)</th>
			<th>1st Ecs</th><th>2nd Esc</th></tr></thead>
			<tfoot><tr><th></th><th>Queue ID</th><th>CSR #</th><th>Owner</th><th>Status</th><th>Description</th>
			<th>Create Time</th><th>Priority</th><th>Remarks</th><th>Actual_Age</th><th>Total_Age </th><th>Adjustments</th><th>Actual Age(hrs)</th>
	...
	while($result2=mysqli_fetch_assoc($query_rows_run))
	{
		...
		issuehis_view.state_id,issuehis_view.change_time,issue_view.create_time,service_view.name as service from
		issuehis_view join issue_view join issuehistype_view join enduser_view join intuser_view join issuestate_view join issuelevel_view join service_view		
		...
		and issue_view.user_id = intuser_view.id and issuelevel_view.id = issue_view.ticket_priority_id and enduser_view.customer_id = issue_view.customer_id and 
		issue_view.service_id=service_view.id 
		where issue_view.create_time>'2016-01-01 00:00:00' and issuehis_view.history_type_id=27
...
$time_remain="";
	...
	while($result=mysqli_fetch_assoc($query_run))
		   ...
		   $total_workingdays=$workingDays+$no_remaining_days;
			...
			if($weekend==6)
				...
				else
					{
						...
						$time_we_rx=strtotime($time_start_rx);
						 $remainh_rx=23-(date("H",$time2)).'<br>';
						 $remainm_rx=59-(date("i",$time2)).'<br>';
						 $remains_rx=60-(date("s",$time2)).'<br>';
						 //$time_start.'<br>';
		...
		echo '<tr><td height="30"><input type="checkbox" name="row['.$rownum.'][0]" value="true" >
		<td height="30"><input type="hidden" name="row['.$rownum.'][1]" value="'.$cx_id.'" />'.$cx_id.'</td>
		  <td height="30"><input type="hidden" name="row['.$rownum.'][2]" value="'.$ticket_no.'" /><a href="https://icare.interblocks.com/otrs/index.pl?Action=AgentTicketZoom;TicketID='.$ticket_id.'"  target="_blank"  rel = "noopener noreferrer">'.$ticket_no.'</a></td>
		 <td height="30" class="name"><input type="hidden" name="row['.$rownum.'][3]" value="'.$owner.'" />'.$owner.' </td>
		 <td height="30"><input type="hidden" name="row['.$rownum.'][4]" value="'.$state.'" />'.$state.'</td>
		 ...
		 <td height="30"><input type="hidden" name="row['.$rownum.'][6]" value="'.$create_time.'" />'.$create_time.'</td>
		 <td height="30"><input type="hidden" name="row['.$rownum.'][7]" value="'.$priority.'" />'.$priority.'</td>
		  <td height="30"><textarea rows="2" cols="40" style=" overflow:auto;resize:none; border: none; font-family:Times New Roman; overflow:hidden; color: green; background-color: transparent;" name="row['.$rownum.'][16]" />'.$remarks.'</textarea></td>
		  
		  	
			<td height="30"><input type="hidden" name="row['.$rownum.'][11]" value="'.$Acual_age_show.'" />'.$Acual_age_show. '</td>
		 <td height="30"><input type="hidden" name="row['.$rownum.'][8]" value="'.$Total_age_show.'" />'.$Total_age_show. '</td>
		 <td height="30"><input type="hidden" name="row['.$rownum.'][9]" value="'.$adjustments.'" />'.$adjustments. '</td>
		 <td height="30"><input type="hidden" name="row['.$rownum.'][10]" value="'.round($Acual_age,1).' hrs" />'.round($Acual_age,1). '</td>';
		 if($result2['1st_esc']=='1'){
		
		   echo'<td height="30"><input type="checkbox"  disabled="disabled" checked="checked" name="row['.$rownum.'][12]" value="true">1st</td>';
	}
	else{
		echo '<td height="30"><input type="checkbox" name="row['.$rownum.'][12]" value="true">1st</td>';
	}
		  if($result2["2nd_esc"]=="1"){
		   echo'<td height="30"><input type="checkbox" disabled="disabled" checked="checked" name="row['.$rownum.'][13]" value="true">2nd</td>';
	}
	else{
		echo '<td height="30"><input type="checkbox" name="row['.$rownum.'][13]" value="true"> 2nd </td>';
	}		 		 	
		echo '</tr>	';
...
//}
	...
	echo '<link rel="stylesheet" type="text/css" href="css/jquery.css"/>
	<script src="jquery/jquery.js"></script>
	<script src="jquery/jquery.dataTables.min.js"></script>';
...
echo '<script type="text/javascript">
			...
			all_csr = all_csr + aa + ",";	
				...
				all_csr = all_csr.substring(1,all_csr.lastIndexOf(","));
				all_csr=all_csr.replace(/,*,/g,",");
				document.getElementById("csrs").value=all_csr;
				//alert(co);
