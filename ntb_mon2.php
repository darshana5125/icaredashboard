	  		...
	  		<div  class="col-md-3">
	  			<table id="table2" class="collaptable table table-danger">
				    ...
				    <tbody>
						...
						u.first_name='Sanjaya' or u.first_name='CS Infra' then count(iv.id) end as csinfra,
						case when u.first_name='Kavinda' or u.first_name='Sujith' or u.first_name='Waruna' then count(iv.id) end as sdi,
                        ...
                        'Suganthan','Priyadarshana','Damith','Interblocks','CS Infra',
                        'Prabath','Sanjaya','Kavinda','Sujith','Geehan','Venura',
				    	...
				    	$sup_ass="select u.first_name as owner,iss.name as state,iv.tn as tn,iv.id,iv.create_time  from issuestate_view iss
							...
							where iv.queue_id=80 and iv.create_time>='2019-01-01'
							and iss.name='Assigned' and iv.customer_id='LKA-NTB' and 
							u.first_name in ('Sanjaya','Prabath','CS Infra')";
						$sup_ass_run=mysqli_query($connection,$sup_ass);
							$data_parent=$data_id;
							while ($sup_ass_result=mysqli_fetch_assoc($sup_ass_run)) {
								$create_date=strtotime($sup_ass_result['create_time']);
				    	...
				    	$sup_ass="select u.first_name as owner,iss.name as state,iv.tn as tn,iv.id,iv.create_time  from issuestate_view iss
							...
							where iv.queue_id=80 and iv.create_time>='2019-01-01'
							and iss.name='Assigned' and iv.customer_id='LKA-NTB' and 
							u.first_name in ('Kavinda','Sujith','Waruna')";
						$sup_ass_run=mysqli_query($connection,$sup_ass);
							$data_parent=$data_id;
							while ($sup_ass_result=mysqli_fetch_assoc($sup_ass_run)) {
								$create_date=strtotime($sup_ass_result['create_time']);
                        ...
                        'Suganthan','Priyadarshana','Damith','Interblocks','CS Infra',
                        'Prabath','Sanjaya','Kavinda','Sujith','Waruna','Geehan','Venura',
